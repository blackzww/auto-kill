...
local cooldown = 1.2 -- segundos entre cada ciclo

local function autoKillLoop()
    if not autoKillEnabled then return end

    -- cooldown para evitar rodar 60x por segundo
    ...
    if nearest and dist < 80 then
        ...
        for i = 1, 4 do
            ...
            if key then
                -- segura tecla mais tempo
                VirtualInputManager:SendKeyEvent(true, key, false, game)
                task.wait(0.45)
                VirtualInputManager:SendKeyEvent(false, key, false, game)
                task.wait(0.5) -- espera antes de mudar para próxima
            end

            -- executa ataques dessa ferramenta
